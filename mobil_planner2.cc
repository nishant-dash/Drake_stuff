  ...
  if (lanes.first != nullptr || lanes.second != nullptr) {
    ...
    else // overtake flag is set, so let ego switch lane and then get ahead
    {
      for (int i = 0; i < traffic_poses.get_num_poses(); ++i)
      {
        ...
        // debugging helper
        // std::cout << "ego: " << s_ego << "   other: " << s_traffic << std::endl;

        if(overtake_to_right == true)
        {
            if(s_ego > s_traffic) //go right when you can
            {
              lane = lanes.second;
              if(old_ego_lane->id().id == ego_position.lane->id().id)
              {
                // means done overtaking, so reset flags and variables used
        ...
        {
            if(s_ego > s_traffic) //go left when you can
            {
              lane = lanes.first;
              if(old_ego_lane->id().id == ego_position.lane->id().id)
              {
