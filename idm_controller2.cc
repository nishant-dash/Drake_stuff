...
template <typename T>
IdmController2<T>::IdmController2(const RoadGeometry& road)
    : road_(road),
      ego_pose_index_{
...
template <typename T>
IdmController2<T>::~IdmController2() {}

template <typename T>
const systems::InputPortDescriptor<T>& IdmController2<T>::ego_pose_input()
    const {
  return systems::System<T>::get_input_port(ego_pose_index_);
}

template <typename T>
const systems::InputPortDescriptor<T>& IdmController2<T>::ego_velocity_input()
    const {
  return systems::System<T>::get_input_port(ego_velocity_index_);
}

template <typename T>
const systems::InputPortDescriptor<T>& IdmController2<T>::traffic_input() const {
  return systems::System<T>::get_input_port(traffic_index_);
}

template <typename T>
const systems::OutputPortDescriptor<T>& IdmController2<T>::acceleration_output()
    const {
  return systems::System<T>::get_output_port(acceleration_index_);
}

template <typename T>
void IdmController2<T>::DoCalcOutput(const systems::Context<T>& context,
...
template <typename T>
void IdmController2<T>::ImplDoCalcOutput(
  ...
  (*command)[0] = IdmPlanner2<T>::Evaluate(idm_params, s_dot_ego, net_distance,
                                          closing_velocity);
}

// These instantiations must match the API documentation in idm_controller.h.
template class IdmController2<double>;
