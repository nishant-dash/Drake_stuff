...
void AddMaliputRailcar(int num_cars, bool idm_controlled, int initial_s_offset,
    ...
    AutomotiveSimulator<double>* simulator, dryvr_data * obj) {
  if(idm_controlled)
  {
  ...
  //for(int i = 0; i < num_cars; ++i)
  //{
      for (int j = 0; j < obj->total_num_cars; j++) {
          if(obj->car_type[j] != 0) continue;
          std::cout << obj->total_num_cars << "FOUND ONEEEEEEEEEEEEEEE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
          ctr ++;
          const int lane_index = obj->lane_number[j];//i % FLAGS_num_dragway_lanes;
          ////////////////////////////////////////////////////////////////////////////
          const double speed = obj->vel_off[j];//FLAGS_dragway_base_speed +
              //lane_index * FLAGS_dragway_lane_speed_delta + FLAGS_maliput_start_speed_offset; // change to 2 for merge behind, 5 otherwise
          ////////////////////////////////////////////////////////////////////////////
          const MaliputRailcarParams<double> params;
          const Lane* lane =
              dragway_road_geometry->junction(0)->segment(0)->lane(lane_index);
          MaliputRailcarState<double> state;
          //const int row = i / FLAGS_num_dragway_lanes + 6;
          ////////////////////////////////////////////////////////////////////////////
          const double s_offset = obj->pos_off[j];//initial_s_offset + kRailcarRowSpacing * row + FLAGS_maliput_start_s_offset;
          ////////////////////////////////////////////////////////////////////////////
          if (s_offset >= lane->length()) {
            throw std::runtime_error(
                "Ran out of lane length to add a MaliputRailcar.");
          ...
          // if (idm_controlled) {
          //   simulator->AddIdmControlledPriusMaliputRailcar(
          //       "IdmControlledMaliputRailcar" + std::to_string(ctr),
          //       LaneDirection(lane), params, state);
          // } else {
            simulator->AddPriusMaliputRailcar("MaliputRailcar" + std::to_string(ctr),
                                              LaneDirection(lane), params, state);
          //}
      }
//    }
...
void AddVehicles(RoadNetworkType road_network_type,
  ...
  if (road_network_type == RoadNetworkType::dragway) {
    ...
    for (int ctr = 0,j = 0; j < obj->total_num_cars; ++j) {
        if(obj->car_type[j] != 1) continue;
        ctr++;
        const int lane_index = obj->lane_number[j];//i % FLAGS_num_dragway_lanes; //change backkkkk
        const std::string name = "MOBIL" + std::to_string(ctr);
        SimpleCarState<double> state;
        const int row = 0;//i / FLAGS_num_dragway_lanes;
        const double x_offset = kControlledCarRowSpacing * row + obj->pos_off[j]; ////// 20 for behind///////
        const Lane* lane =
            dragway_road_geometry->junction(0)->segment(0)->lane(lane_index);
        if (x_offset >= lane->length()) {
          throw std::runtime_error(
        ...
        state.set_x(x_offset);
        state.set_y(y_offset);
        state.set_velocity(obj->vel_off[j]);  ////////// to 6 for behind, otherswise ///////////////// changes/////////////
        simulator->AddMobilControlledSimpleCar(name, true /* with_s */, state);
      }
  //}

  //for (int i = 0; i < FLAGS_num_mobil2_car; ++i) {
    for (int ctr = 0,j = 0; j < obj->total_num_cars; ++j) {
        if(obj->car_type[j] != 2) continue;
        ctr++;
        const int lane_index = obj->lane_number[j];//i % FLAGS_num_dragway_lanes;
        const std::string name = "MOBIL2" + std::to_string(ctr);
        SimpleCarState<double> state;
        const int row = 0;//i / FLAGS_num_dragway_lanes;
        const double x_offset = kControlledCarRowSpacing * row + obj->pos_off[j];
        const Lane* lane =
            dragway_road_geometry->junction(0)->segment(0)->lane(lane_index);
        if (x_offset >= lane->length()) {
          throw std::runtime_error(
              "Ran out of lane length to add new MOBIL2-controlled SimpleCars.");
        }
        const double y_offset = lane->ToGeoPosition({0., 0., 0.}).y();
        state.set_x(x_offset);
        state.set_y(y_offset);
        state.set_velocity(obj->vel_off[j]);
        simulator->AddMobil2ControlledSimpleCar(name, true /* with_s */, state);
      }
  //}

  //for (int i = 0; i < FLAGS_num_mobil3_car; ++i) {
    for (int ctr = 0,j = 0; j < obj->total_num_cars; ++j) {
        if(obj->car_type[j] != 3) continue;
        ctr++;
          const int lane_index =  + obj->lane_number[j];//i % FLAGS_num_dragway_lanes;
          const std::string name = "MOBIL3" + std::to_string(ctr);
          SimpleCarState<double> state;
          const int row = 0;//i / FLAGS_num_dragway_lanes; ////// zero for behind
          const double x_offset = kControlledCarRowSpacing * row + obj->pos_off[j]; //////// ////// zero for behind/////
          const Lane* lane =
              dragway_road_geometry->junction(0)->segment(0)->lane(lane_index);
          if (x_offset >= lane->length()) {
            throw std::runtime_error(
                "Ran out of lane length to add new MOBIL3-controlled SimpleCars.");
          }
          const double y_offset = lane->ToGeoPosition({0., 0., 0.}).y();
          state.set_x(x_offset);
          state.set_y(y_offset);
          state.set_velocity(obj->vel_off[j]); //////// change to 4 for merge behind/ 2 otherswise////////// cahnges //////////////
          simulator->AddMobil3ControlledSimpleCar(name, true /* with_s */, state);
        }
  //  }

    AddMaliputRailcar(FLAGS_num_idm_controlled_maliput_railcar,
        true /* IDM controlled */, 0 /* initial s offset */,
        dragway_road_geometry, simulator, obj);
    ...
    AddMaliputRailcar(FLAGS_num_maliput_railcar, false /* IDM controlled */,
        initial_s_offset, dragway_road_geometry, simulator, obj);
...
bool parse_file_sim_data(dryvr_data * obj) {

  std::string dir_string("/tmp/drake-dryvr/sim1.txt");
  std::fstream file_sim_data(dir_string, std::ios_base::in);
  //if(file_sim_data == NULL) return false; //file not found

  // some temp vars
  double info;
  int i=0, format_number=0, ACK=0, num_mali=0, num_mobil=0;

  // read meta-data
  file_sim_data >> format_number;
  file_sim_data >> ACK;

  // read global simulation definition
  file_sim_data >> FLAGS_num_dragway_lanes;
  file_sim_data >> FLAGS_simulation_sec;
  file_sim_data >> obj->total_num_cars;

  // read specific simulation defition
  while (file_sim_data >> info)
  {
    if(i==0)
    {
      obj->car_type.push_back(int(info));
      if(info == 0) num_mali++;
      else if(info == 1) num_mobil++;
      //else if(info == 2) FLAGS_num_mobil2_car++;
      //else if(info == 3) FLAGS_num_mobil3_car++;
    }
    else if(i==1) obj->car_num.push_back(int(info));//FLAGS_car_number = int(info);
    else if(i==2) obj->pos_off.push_back(info);//FLAGS_maliput_start_s_offset = info;
    else if(i==3) obj->vel_off.push_back(info);//FLAGS_maliput_start_speed_offset = info;
    else obj->lane_number.push_back(int(info));

    if(i == 4) i=0; // reset count
    else i++;
  }
  file_sim_data.close();

  //FLAGS_num_maliput_railcar = num_mali;
  //FLAGS_num_mobil_car = num_mobil;
  std::cout << num_mobil + num_mali;

  std::string dir_string2("/tmp/drake-dryvr/ack.txt");
  std::fstream file_sim_data2(dir_string2, std::ios_base::out);
  file_sim_data2 << ACK;
  file_sim_data2.close();
...
int main(int argc, char* argv[]) {
  ...
  dryvr_data * sim_obj = new dryvr_data;
  parse_file_sim_data(sim_obj);

  //std::cout << FLAGS_num_maliput_railcar << std::endl;
  //std::cout << FLAGS_target_realtime_rate << std::endl; // maybe something to set by self? TODO:
  ...
  AddVehicles(road_network_type, road_geometry, simulator.get(), sim_obj);
