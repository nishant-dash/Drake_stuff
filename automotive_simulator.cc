...
int AutomotiveSimulator<T>::AddMobilControlledSimpleCar(
  ...
  auto idm_controller = builder_->template AddSystem<IdmController2<T>>(*road_);//////// 2 to  ///////////////////////////////////////////////////////////////////
...
template <typename T>
int AutomotiveSimulator<T>::AddMobil2ControlledSimpleCar(
    const std::string& name, bool initial_with_s,
    const SimpleCarState<T>& initial_state) {
  DRAKE_DEMAND(!has_started());
  DRAKE_DEMAND(aggregator_ != nullptr);
  ...
  if (road_ == nullptr) {
    throw std::runtime_error(
        "AutomotiveSimulator::AddMobilControlledSimpleCar2(): "
        "RoadGeometry not set. Please call SetRoadGeometry() first before "
        "calling this method.");
  }
  const int id = allocate_vehicle_number();
  ...
  auto mobil_planner =
      builder_->template AddSystem<MobilPlanner2<T>>(*road_, initial_with_s);
  mobil_planner->set_name(name + "_mobil_planner2");
...
template <typename T>
int AutomotiveSimulator<T>::AddMobil3ControlledSimpleCar(
    const std::string& name, bool initial_with_s,
    const SimpleCarState<T>& initial_state) {
  DRAKE_DEMAND(!has_started());
  DRAKE_DEMAND(aggregator_ != nullptr);
  ...
  if (road_ == nullptr) {
    throw std::runtime_error(
        "AutomotiveSimulator::AddMobil3ControlledSimpleCar(): "
        "RoadGeometry not set. Please call SetRoadGeometry() first before "
        "calling this method.");
  }
  const int id = allocate_vehicle_number();
  ...
  auto mobil_planner =
      builder_->template AddSystem<MobilPlanner3<T>>(*road_, initial_with_s);
  mobil_planner->set_name(name + "_mobil_planner3");
  auto idm_controller = builder_->template AddSystem<IdmController<T>>(*road_);
  idm_controller->set_name(name + "_idm_controller");
