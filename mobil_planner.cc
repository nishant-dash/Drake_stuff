...
void MobilPlanner<T>::ImplDoCalcLane(
  ...
  if (lanes.first != nullptr || lanes.second != nullptr) {
    ...
    */
    ...
    //if(ego_accel_command[0] <= 0)
    //  lane = ego_position.lane;
    //else
      //lane = (incentives.second > incentives.first) ? lanes.second : ego_position.lane;
    //{


    // MERGE BEHIND
      // const double& s_ego = ego_position.pos.s(); // ego car
      // double s_min = 100;
      ...
      //   //  continue; // since it's not the car we're looking for
      //
      ...
      // try to switch to right lane, i.e, MERGE BETWEEN, comment out for MERGE BEHIND
      //  if(incentives.second > incentives.first) lane = lanes.second;
      //  else lane = ego_position.lane;
      ...
      for (int i = 0; i < traffic_poses.get_num_poses(); ++i)
      {
        ...
        if(s_traffic != s_ego)
        {
          s_cars[ctr] = s_traffic;
          ctr = ctr<1? ctr+1 : 0;
        }
      }

      if((s_ego < s_cars[0] && s_ego > s_cars[1]) || (s_ego > s_cars[0] && s_ego < s_cars[1]))
        lane = (incentives.second > incentives.first) ? lanes.second : ego_position.lane;
      else
        lane = ego_position.lane;
